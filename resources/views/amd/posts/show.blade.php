...
@section('title', $post->meta_title_for_seo)
...
<header class="relative py-16 md:py-24 overflow-hidden border-b border-slate-800/50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div class="flex items-center gap-2 mb-4">
            <a href="{{ route('posts.index') }}" class="text-slate-400 hover:text-white transition-colors">Tin tức</a>
            @if($post->category)
...
<section class="py-16 bg-slate-950/50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 class="text-2xl font-bold mb-12 text-center">Bài viết liên quan</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            @foreach($relatedPosts as $related)
                <a href="{{ route('posts.show', $related->slug) }}" class="group block bg-slate-900/50 border border-slate-800 rounded-2xl overflow-hidden hover:border-primary/50 transition-all">
                    <div class="aspect-video overflow-hidden relative">
                        <img src="{{ $related->image ? asset('storage/' . $related->image) : asset('images/placeholder-project.svg') }}" alt="{{ $related->title }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                        @if($related->category)
                    ...
                    <div class="p-6">
                        <h3 class="font-bold text-white group-hover:text-primary transition-colors line-clamp-2">{{ $related->title }}</h3>
                    </div>
                </a>
            @endforeach
        </div>
